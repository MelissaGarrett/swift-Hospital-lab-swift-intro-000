...
extension Payable {
    // DON'T have to implement here in extension; a default
    func wages() -> Double {
        return 50_000.00
    }
}
...
extension TimeOff {
    // default implementation of requestForVacation()
    // so don't need to implement this function in structs/classes that
    // extend TimeOff
    func requestForVacation(_ days: Int) -> Bool {
        if self.vacationDays >= days {
            return true
        }
        return false
    }
...
// Created this since all employee types in SchoolEmployees.swift
// conform to both Payable and TimeOff
